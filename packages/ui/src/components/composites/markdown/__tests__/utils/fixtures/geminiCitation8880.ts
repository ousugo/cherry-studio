 ...
 */
// Structural minimal types — fixture only uses GroundingSupport-shaped data.
// Index signatures allow the original Gemini shape (webSearchQueries etc.)
// to pass through without pulling @google/genai as a dep.
import type { GroundingSupportLike as GroundingSupport } from '../../../utils/citation'
type GroundingChunk = { web?: { uri?: string; title?: string }; [k: string]: unknown }
...
export function buildContent(): string {
  ...
  for (const support of groundingSupports) {
    const text = support.segment?.text as string | undefined
    if (!text) continue
    const start = (support.segment?.startIndex as number | undefined) ?? 0
    buffer.set(encoder.encode(text), start)
  }
