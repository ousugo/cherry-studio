...
vi.mock('@renderer/hooks/useShortcuts', () => ({
  useShortcut: (key: string, handler: () => void, options?: Record<string, unknown>) => {
...
describe('AgentComposer', () => {
  ...
  it('passes the chat model shortcut to the model selector', () => {
    ...
    expect(screen.getByTestId('agent-model-selector')).toHaveAttribute('data-shortcut', 'chat.select_model')
  })
  ...
  it('routes new session shortcuts through the explicit parent action', () => {
    ...
    expect(mocks.shortcutOptions.get('topic.new')).toEqual(
      expect.objectContaining({ preventDefault: true, enableOnFormTags: true })
    )
  })
