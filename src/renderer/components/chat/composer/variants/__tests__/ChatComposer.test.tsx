...
vi.mock('@renderer/components/chat/composer/ComposerToolRuntime', () => ({
  ...
  useComposerTokenReconcile: () => (draftTokens: readonly ComposerSerializedToken[]) => {
    const tokenIds = new Set(draftTokens.map((token) => token.id))
    const knowledgeTokenIds = new Set(draftTokens.flatMap((token) => (token.kind === 'knowledge' ? [token.id] : [])))

    mocks.setFiles((previousFiles: any[]) => previousFiles.filter((file) => tokenIds.has(`file:${file.id}`)))
    mocks.setSelectedKnowledgeBases((previousBases: KnowledgeBase[]) => {
      ...
      const nextTokenIds = new Set(nextBases.map((base) => `knowledge:${base.id}`))
      let changed = nextBases.length !== previousBases.length

      for (const base of mocks.knowledgeBases) {
        const tokenId = `knowledge:${base.id}`
        if (!knowledgeTokenIds.has(tokenId) || nextTokenIds.has(tokenId)) continue
        nextBases.push(base)
