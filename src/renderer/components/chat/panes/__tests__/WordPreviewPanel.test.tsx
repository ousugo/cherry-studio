...
describe('WordPreviewPanel', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.wordReadPreview.mockResolvedValue({ data: new Uint8Array([1, 2, 3]), success: true })
  ...
  it('reads the Word document and renders it through docx-preview', async () => {
    ...
    expect(mocks.wordReadPreview).toHaveBeenCalledWith({
      filePath: 'proposal.docx',
      workspacePath: '/tmp/workspace'
    })
  ...
  it('shows a structured error state when docx rendering fails', async () => {
    ...
    render(<WordPreviewPanel workspacePath="/tmp/workspace" filePath="proposal.docx" fileName="proposal.docx" />)
  ...
  it('shows a read error state when the source file cannot be loaded', async () => {
    mocks.wordReadPreview.mockResolvedValueOnce({
      error: { code: 'word_read_failed', message: 'missing' },
      success: false
    })

    render(<WordPreviewPanel workspacePath="/tmp/workspace" filePath="missing.docx" fileName="missing.docx" />)
  ...
  it('ignores stale render completions after the preview source changes', async () => {
    const renderJobs: Array<{ bodyContainer: HTMLElement; resolve: () => void; text: string }> = []
    mocks.wordReadPreview
      .mockResolvedValueOnce({ data: new Uint8Array([1]), success: true })
      .mockResolvedValueOnce({ data: new Uint8Array([2]), success: true })
    mocks.renderAsync.mockImplementation(async (data: Uint8Array, bodyContainer: HTMLElement) => {
      const text = data[0] === 1 ? 'First document' : 'Second document'
    ...
    const { rerender } = render(
      <WordPreviewPanel workspacePath="/tmp/workspace" filePath="first.docx" fileName="first.docx" />
    )
    await waitFor(() => expect(renderJobs).toHaveLength(1))

    rerender(<WordPreviewPanel workspacePath="/tmp/workspace" filePath="second.docx" fileName="second.docx" />)
    await waitFor(() => expect(renderJobs).toHaveLength(2))

    renderJobs[1].resolve()
    await waitFor(() => expect(screen.getByTestId('word-preview-document')).toHaveTextContent('Second document'))

    renderJobs[0].resolve()
    await waitFor(() => expect(screen.getByTestId('word-preview-document')).toHaveTextContent('Second document'))
    expect(screen.getByTestId('word-preview-document')).not.toHaveTextContent('First document')
  })
})
