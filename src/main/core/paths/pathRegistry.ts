 ...
 * other than `PATHS` in this file. The ESLint rule `data-schema-key/valid-key`
 * has no AST scope and validates every property name in the file; helper
 * objects (e.g. `{ darwin: '...', win32: '...' }`) trigger false-positive
 * errors. Helper constants must be `string` or `number` only. If you need a
 * helper object, put it in a separate file.
 */
...
/** Frozen registry of all known paths. */
