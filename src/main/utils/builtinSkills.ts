...
async function syncBuiltinSkillToDb(folderName: string, destPath: string, filesUpdated: boolean): Promise<void> {
  try {
    const existing = await skillRepository.getByFolderName(folderName)
    ...
    if (existing) {
      ...
      await skillRepository.updateMetadata(existing.id, {
        name: metadata.name,
      ...
      // id/createdAt/updatedAt are auto-generated by schema helpers.
      const inserted = await skillRepository.insert({
        name: metadata.name,
        description: metadata.description ?? null,
        folderName,
        source: 'builtin',
